...
describe('test for qna pages',()=>{
    ...
    async function getSearchResultOfFirst(){
        await page.waitFor(1000)
        await page.waitFor(()=>document.querySelector('div.mq-card'))
        await page.waitFor(1000)
        return (await page.evaluate(()=>{
            return document.querySelector('h4.mq').textContent           
        }))
...
//######################################################INITALIZING BROWSER FOR RUNNING TEST CASES#######################################################
    beforeAll(async (done)=>{
        ...
        await page.goto(cred[0],{waitUntil:'load'})
        await page.type('input[name=username]',cred[1])
        await page.type('input[name=password]',cred[2])
...
//########################################################TESTS BEGIN FROM HERE#########################################################################
    ...
    test('testing for search',async (done)=>{
        await page.reload({waitUntil:'load'})
        await page.waitFor(2000)
        await search('can i bring')
        expect(await getSearchResultOfFirst()).toEqual('Can I bring pets to work?')
        done()
    })
